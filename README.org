...
- RRUL: The standard chart used by the Bufferbloat project to show the down/upload speeds, with the latency in three separate charts. =netperf-wrapper rrul -p all_scaled -l 60 -H atl.richb-hanover.com -t text-to-be-included-in-plot=
- TCP Upload: Displays TCP upload speed and latency in two charts. =netperf-wrapper tcp_upload -p totals -l 60 -H atl.richb-hanover.com -t text-to-be-included-in-plot=
- TCP Download: Displays TCP download speeds and latency in two charts. =netperf-wrapper tcp_download -p totals -l 60 -H atl.richb-hanover.com -t text-to-be-included-in-plot=
