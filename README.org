...
Netperf needs to be started in "server mode" on one computer. To do this:

    =netserver &=

Netperf will be installed automatically when you install netperf-wrapper on the other computer. 
There are several interesting netperf-wrapper commands:
